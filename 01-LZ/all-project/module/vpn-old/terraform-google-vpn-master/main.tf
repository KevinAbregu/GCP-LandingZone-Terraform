...
resource "google_compute_route" "route" {
  count      = !var.cr_enabled ? length(var.remote_subnet_map[0]) + length(var.remote_subnet_map[1]) : 0
  name       = "${google_compute_vpn_gateway.vpn_gateway.name}-tunnel${floor(count.index ) + 1}-route${count.index + 1}"
  network    = var.network
  project    = var.project_id
  dest_range = var.remote_subnet_map[count.index][0]
  priority   = var.route_priority
  tags       = var.route_tags

  next_hop_vpn_tunnel = google_compute_vpn_tunnel.tunnel-static[floor(count.index)].self_link
